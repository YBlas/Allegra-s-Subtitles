...
const App = () => {
  ...
  const parseSRT = (srt: string): SRTEntry[] => {
    return srt
      ...
      .map(block => {
        ...
        const text = lines.slice(2).join('\n'); // join multiline subtitles
        return { id, start, end, text };
      })
      .filter(entry => !isNaN(entry.id) && entry.start && entry.end);
  }
  ...
  useEffect(() => { srtTextRef.current = srtText }, [srtText]);

  const pressSpace = () => {
    if (manualModeRef.current) {
      if (currentTextRef.current !== undefined && srtTextRef.current.length > 0) {
        const currentIndex = srtTextRef.current.findIndex(entry => entry.id === currentTextRef.current);
    ...
    <ContainerWeb>
      {!currentText ? <>
        ...
        <ul>
          ...
          <li>Press 'M' to toggle manual mode. Manual mode will trigger down all timing functionality, use it carefully.</li>
          <li>In manual mode, press 'B' to toggle the previous/next buttons.</li>
      ...
      <>
        {showPrevNext && manualMode && <ButtonsContainer>
          <button onClick={() => {
            if(srtText.findIndex(entry => entry.id === currentText) > 0) {
              setCurrentText(srtText.at(srtText.findIndex(entry => entry.id === currentText) - 1)?.id);
            }
          }} disabled={currentText <= 0}>
            Previous
          </button>
          <button onClick={() => {
            if(srtText.findIndex(entry => entry.id === currentText) < srtText.length - 1) {
              setCurrentText(srtText.at(srtText.findIndex(entry => entry.id === currentText) + 1)?.id);
            }
        ...
        <SubtitleText marginBottom={marginBottomSubtitle}>
          <div dangerouslySetInnerHTML={{ __html: srtText.find(entry => entry.id === currentText)?.text ?? "" }} />
        </SubtitleText>
      </>}
    </ContainerWeb>
  )
